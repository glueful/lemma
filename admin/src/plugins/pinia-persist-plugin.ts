...
export default async ({ options, store }: PiniaPluginContext): Promise<void> => {
  ...
  const restored: Promise<void> = Promise.all(
    strategies.map(async (strategy) => {
      try {
        ...
        if (strategy.serializer) {
          parsedState = strategy.serializer.deserialize(stateFromStorage) as Record<string, unknown>
        } else {
  ...
  // Pinia does NOT await plugins, and this restore is async (Web Crypto decrypt). Expose the
  // in-flight promise on the store so app bootstrap can await hydration BEFORE mount — otherwise a
  // hard refresh runs the router's auth guard against an empty session and bounces a logged-in user
  // to /login.
  ;(store as unknown as { $persistRestored?: Promise<void> }).$persistRestored = restored
